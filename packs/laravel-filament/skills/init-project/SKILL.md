...
- Diretório alvo pode estar vazio ou conter apenas o scaffold ai-first: `.agents/`, `.git/`, `.cert/`, `.git-crypt-keys/`, `.gitattributes`, `.gitignore`, `README.md`. Se houver `composer.json`, `vendor/` ou artefato Laravel, tratar como instalação parcial e seguir `rules/partial-install-recovery.md` antes de continuar.
