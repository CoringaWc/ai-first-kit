...
- **Global scope**: `/home/coringawc/.config/opencode/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/coringawc/.config/opencode/notes/`)
...
2. Glob `/home/coringawc/.config/opencode/notes/*.md` (if directory exists) — global notes
...
Global (/home/coringawc/.config/opencode/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/coringawc/.config/opencode/notes/` — works in any directory
