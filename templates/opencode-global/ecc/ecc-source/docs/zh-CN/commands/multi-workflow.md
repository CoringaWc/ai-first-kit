...
# 新会话调用
Bash({
  command: "/home/coringawc/.config/opencode/ccg/bin/codeagent-wrapper {{LITE_MODE_FLAG}}--backend <codex|gemini> {{GEMINI_MODEL_FLAG}}- \"$PWD\" <<'EOF'
ROLE_FILE: <角色提示文件路径>
<TASK>
...
# 恢复会话调用
Bash({
  command: "/home/coringawc/.config/opencode/ccg/bin/codeagent-wrapper {{LITE_MODE_FLAG}}--backend <codex|gemini> {{GEMINI_MODEL_FLAG}}resume <SESSION_ID> - \"$PWD\" <<'EOF'
ROLE_FILE: <角色提示文件路径>
<TASK>
需求: <增强后的需求（如未增强则为$ARGUMENTS）>
上下文: <来自先前阶段的项目上下文和分析>
