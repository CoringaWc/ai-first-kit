---
description: "CCG Multi-Agent: Executa workflow completo com pesquisa, plano, implementacao, otimizacao e review."
---

# Workflow - Multi-Model Collaborative Development

Multi-model collaborative development workflow (Research -> Ideation -> Plan -> Execute -> Optimize -> Review), using Codex for all external model calls in this environment.

## Environment Override

- Gemini is not installed here. Never call the Gemini backend.
- Use `--backend codex` for frontend, backend, and review external-model calls.
...
- Multi-model collaboration: Codex (external analysis/review) + Claude (orchestration)
...
- **Codex** – Frontend/backend analysis, planning, implementation review
...
**Background Calls**: Use `run_in_background: true` to start, wait for results with `TaskOutput`. **Must wait for the external model result before proceeding to next phase**.
...
```bash
node scripts/orchestrate-worktrees.js .opencode/plan/workflow-e2e-test.json --execute
```
...
1. **Prompt Enhancement** (if ace-tool MCP available): Call `mcp__ace-tool__enhance_prompt`, **replace original $ARGUMENTS with enhanced result for all subsequent Codex calls**. If unavailable, use `$ARGUMENTS` as-is.
...
**External Call**:
- Codex: Use analyzer prompt, output technical feasibility, UI feasibility, solutions, UX evaluation, and risks

Wait for results with `TaskOutput`. **Save SESSION_ID** (`CODEX_SESSION`).
...
**External Call** (resume session with `resume <SESSION_ID>`):
- Codex: Use architect prompt + `resume $CODEX_SESSION`, output backend and frontend architecture

Wait for results with `TaskOutput`.

**Follow the `IMPORTANT` instructions in `Multi-Model Call Specification` above**

**Claude Synthesis**: Adopt the Codex plan, refine it with local project context, and save to `.opencode/plan/task-name.md` after user approval.
...
**External Call**:
- Codex: Use reviewer prompt, focus on security, performance, error handling, accessibility, and design consistency
