---
description: "CCG Multi-Agent: Cria plano multi-modelo sem modificar codigo de producao."
---
...
- **Mandatory Background Call**: Codex calls MUST use `run_in_background: true` (including single model calls, to avoid blocking main thread)
- **Code Sovereignty**: External models have **zero filesystem write access**, all modifications by Claude
- **Stop-Loss Mechanism**: Do not proceed to next phase until current phase output is validated
- **Planning Only**: This command allows reading context and writing to `.opencode/plan/*` plan files, but **NEVER modify production code**
...
- Gemini-specific flags are disabled in this environment. Use Codex with no Gemini model flag.
...
**Session Reuse**: Each call returns `SESSION_ID: xxx` (typically output by wrapper), **MUST save** for subsequent `/multi-execute` use.
...
**Codex call** (`run_in_background: true`):

Distribute **original requirement** (without preset opinions) to Codex:
...
Wait for the complete result with `TaskOutput`. **Save SESSION_ID** (`CODEX_SESSION`).
...
3. **Complementary checks**: Ask Codex to cover backend logic, frontend design, UX, and accessibility
4. **Logical reasoning**: Eliminate logical gaps in solutions

#### 2.3 (Optional but Recommended) Dual-Model Plan Draft

To reduce risk of omissions in Claude's synthesized plan, can ask Codex for a separate "plan draft" (still **NOT allowed** to modify files):
...
Wait for the complete result with `TaskOutput`, then record key risks and tradeoffs.
...
### Technical Solution
<Optimal solution synthesized from Codex analysis and local project context>
...
**`/multi-plan` responsibilities end here, MUST execute the following actions**:

1. Present complete implementation plan to user (including pseudo-code)
2. Save plan to `.opencode/plan/<feature-name>.md` (extract feature name from requirement, e.g., `user-auth`, `payment-module`)
3. Output prompt in **bold text** (MUST use actual saved file path):

---
**Plan generated and saved to `.opencode/plan/actual-feature-name.md`**
...
```
/multi-execute .opencode/plan/actual-feature-name.md
```
---
...
- Ask user "Y/N" then auto-execute (execution is `/multi-execute`'s responsibility)
- Any write operations to production code
- Automatically call `/multi-execute` or any implementation actions
...
- **First planning**: `.opencode/plan/<feature-name>.md`
- **Iteration versions**: `.opencode/plan/<feature-name>-v2.md`, `.opencode/plan/<feature-name>-v3.md`...
...
2. Update `.opencode/plan/<feature-name>.md` file
...
```bash
/multi-execute .opencode/plan/<feature-name>.md
```
...
3. **Trust Rules** – Codex is the only configured external model in this environment
4. External models have **zero filesystem write access**
5. **SESSION_ID Handoff** – Plan must include `CODEX_SESSION` at end (for `/multi-execute resume <SESSION_ID>` use)
