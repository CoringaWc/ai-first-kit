...
try {
  ...
  const MODEL_ACTIONS = [
    ...
    { keywords: ['gemini审查', 'gemini 审查', 'gemini review', '用gemini看', '让gemini检查'], model: 'codex', role: 'reviewer', action: 'Gemini is unavailable here; use Codex to审查当前代码变更（git diff）' },
    { keywords: ['gemini分析', 'gemini 分析', 'gemini analyze', '用gemini分析'], model: 'codex', role: 'analyzer', action: 'Gemini is unavailable here; use Codex to分析当前项目/代码' },
    { keywords: ['gemini前端', 'gemini 前端', '用gemini做前端'], model: 'codex', role: 'architect', action: 'Gemini is unavailable here; use Codex for前端开发分析' },
  ...
  if (modelAction) {
    ...
    const wrapperPath = path.join(homeDir, '.config', 'opencode', 'ccg', 'bin', 'codeagent-wrapper');
...
2. 并行调用两个模型 (run_in_background: true):
   ...
   ROLE_FILE: ${path.join(homeDir, '.config', 'opencode', 'ccg', '.ccg', 'prompts', 'codex', modelAction.role + '.md')}
   <TASK>${modelAction.action}</TASK>
   EOF

   Frontend/design pass (codex, Gemini unavailable):
   ${wrapperPath} --progress --backend codex - "$WORKDIR" <<'EOF'
   ROLE_FILE: ${path.join(homeDir, '.config', 'opencode', 'ccg', '.ccg', 'prompts', 'codex', modelAction.role + '.md')}
   <TASK>${modelAction.action}</TASK>
   EOF
...
2. 调用模型:
   ...
   ROLE_FILE: ${path.join(homeDir, '.config', 'opencode', 'ccg', '.ccg', 'prompts', modelAction.model, modelAction.role + '.md')}
   <TASK>${modelAction.action}</TASK>
   EOF
...
3. 等待结果并输出
  ...
  const homeDir = process.env.HOME || process.env.USERPROFILE || '';
  const skillsBase = path.join(homeDir, '.config', 'opencode', 'ccg', 'skills', 'ccg');
