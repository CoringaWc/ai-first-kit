...
```
Read("/home/coringawc/.config/opencode/ccg/.ccg/engine/model-router.md")
```
...
2. **Codex** — 外部模型写代码，更快更便宜，Claude 监控审查
...
Claude 作为编排者，调用外部模型 Codex **并行写代码**。
...
- 每个子任务的文件范围**不可重叠**
- 可混合 backend（当前环境仅使用 codex）— 不要指定不可用 backend
