...
### Phase 3: Codex 分析 [required]

**Gate check**: 需求已增强 ✓ 上下文已收集 ✓

**⛔ M 复杂度必须调用 Codex 做后台分析。Gemini/Antigravity 在当前环境不可用。不可跳过。**

核心价值是让外部模型从后端与前端/集成两个角度分析同一个问题，弥补单一路径盲区。

执行步骤：

1. 确定工作目录：`WORKDIR=$(pwd)`

2. **调用 Codex 分析**（`run_in_background: true`）：

Codex 后端视角：
```
Bash({
  command: "/home/coringawc/.config/opencode/ccg/bin/codeagent-wrapper --progress --backend codex - \"$WORKDIR\" <<'CODEAGENT_EOF'\nROLE_FILE: /home/coringawc/.config/opencode/ccg/.ccg/prompts/codex/analyzer.md\n<TASK>\n需求：{增强后的需求}\n上下文：{Phase 2 收集的项目上下文、相关代码摘要}\n</TASK>\nOUTPUT: 技术分析报告（可行性、架构建议、风险评估、实施方案对比）\nCODEAGENT_EOF",
  run_in_background: true,
  timeout: 3600000,
  description: "Codex 后端视角分析"
})
```
...
2. **Codex** — 外部模型写代码，更快，Claude 监控审查
...
```
Bash({
  command: "/home/coringawc/.config/opencode/ccg/bin/codeagent-wrapper --progress --parallel --backend codex - \"$WORKDIR\" <<'PARALLEL_EOF'\n---TASK---\nid: layer1-{name1}\nworkdir: $WORKDIR\n---CONTENT---\nROLE_FILE: /home/coringawc/.config/opencode/ccg/.ccg/prompts/codex/builder.md\n<TASK>\n## 文件范围（⛔ 只改这些文件）\n{file1, file2}\n\n## 实施步骤\n{steps from plan.md}\n</TASK>\n---TASK---\nid: layer1-{name2}\nworkdir: $WORKDIR\n---CONTENT---\nROLE_FILE: /home/coringawc/.config/opencode/ccg/.ccg/prompts/codex/builder.md\n<TASK>\n## 文件范围\n{file3, file4}\n\n## 实施步骤\n{steps}\n</TASK>\n---TASK---\nid: layer2-{name3}\nworkdir: $WORKDIR\ndependencies: layer1-{name1},layer1-{name2}\n---CONTENT---\nROLE_FILE: /home/coringawc/.config/opencode/ccg/.ccg/prompts/codex/builder.md\n<TASK>\n## 文件范围\n{file5}\n\n## 实施步骤\n{steps}\n</TASK>\nPARALLEL_EOF",
  run_in_background: true,
  timeout: 3600000,
