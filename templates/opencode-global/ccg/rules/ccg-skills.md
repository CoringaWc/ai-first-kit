...
When working in a project, automatically invoke the corresponding quality gate skills based on the scenario below. These skills are installed at `/home/coringawc/.config/opencode/ccg/skills/ccg/` and can be called directly.

**IMPORTANT**: OpenCode skill names use the `ccg-` prefix, not the legacy `ccg:` namespace. Use exactly: `ccg-verify-security`, `ccg-verify-quality`, `ccg-verify-change`, `ccg-verify-module`, `ccg-gen-docs`. Do NOT call `ccg:verify-security` — that will fail with "Unknown skill".
...
When a task involves 3+ independent files/modules or 2+ parallel workflows, refer to the multi-agent orchestration skill at `/home/coringawc/.config/opencode/ccg/skills/ccg/orchestration/ccg-multi-agent/SKILL.md` for:
