...
function rewriteOpencodeEccSkillName(content, destinationPath) {
  const skillName = path.basename(path.dirname(destinationPath));
...
function applyInstallPlan(plan) {
  ...
  for (const operation of plan.operations) {
    ...
    if (
      operation.transform === 'opencode-ecc-skill-prefix'
      && path.basename(operation.destinationPath) === 'SKILL.md'
    ) {
      const content = fs.readFileSync(operation.sourcePath, 'utf8');
      fs.writeFileSync(
        operation.destinationPath,
        rewriteOpencodeEccSkillName(content, operation.destinationPath),
        'utf8'
      );
      continue;
    }
