...
description: "Global: Use para qualidade de código em tempo de escrita com Plankton, incluindo autoformatação, lint e correções assistidas por Claude em edições de arquivos."
origin: community
---
...
No install command, no plugin config. The hooks in `~/.config/opencode/opencode.jsonc` are picked up automatically when you run Claude Code in the Plankton directory.

### Per-Project Integration

To use Plankton hooks in your own project:

1. Copy `~/.config/opencode/hooks/` directory to your project
2. Copy `~/.config/opencode/opencode.jsonc` hook configuration
...
Plankton's `~/.config/opencode/hooks/config.json` controls all behavior:
