---
name: skill-scout
description: "Global: Use quando o usuário quiser criar, buscar, encontrar ou bifurcar uma skill para um workflow; pesquisa fontes locais, marketplace, GitHub e web antes de criar algo novo."
origin: community
---
...
```bash
find ~/.config/opencode/skills -maxdepth 2 -name SKILL.md 2>/dev/null | grep -iE "keyword|synonym"
find ~/.config/opencode/external -path '*/skills/*/SKILL.md' 2>/dev/null | grep -iE "keyword|synonym"
```

Then search frontmatter descriptions:

```bash
grep -RilE "keyword|synonym" ~/.config/opencode/skills ~/.config/opencode/external 2>/dev/null
```
